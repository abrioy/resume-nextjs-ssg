...
(async () => {
    ...
    for (const variant of configuration.variants) {
        console.log(`Creating variant ${variant.locale.htmlLang}`);
        for (const document of variant.documents) {
            await makePDF(
                browser,
                `${APPLICATION_URL}#${document.anchor}`,
                `${PDF_OUTPUT_PATH}/${document.pdfName}`,
            );
        }
        ...
        await makePreview(
            ...
            `${PREVIEW_OUTPUT_PATH}/preview_${variant.locale.url}.png`,
        );
    }
